...
export const distributionConfig: DistributionConfig = Object.freeze({
  client: {
    defaultVersionId: process.env.SHINDO_CLIENT_DEFAULT_VERSION_ID?.trim() || DEFAULT_CLIENT_VERSION_ID,
    cdnBaseUrl: normalizeBaseUrl(process.env.SHINDO_CDN_BASE_URL, DEFAULT_CDN_BASE_URL),
    cdnManifestCandidates: parseList(process.env.SHINDO_CDN_MANIFESTS, DEFAULT_CDN_MANIFEST_CANDIDATES),
    ...
    defaultVersionAssetName: process.env.SHINDO_CLIENT_VERSION_ASSET?.trim() || DEFAULT_VERSION_ASSET_NAME,
    defaultPackageAssetName: process.env.SHINDO_CLIENT_PACKAGE_ASSET?.trim() || DEFAULT_PACKAGE_ASSET_NAME,
  },
  java: {
...
export function resolveClientRepo(versionId: string): string {
  return distributionConfig.client.githubRepoMap[versionId] || distributionConfig.client.githubDefaultRepo
}
